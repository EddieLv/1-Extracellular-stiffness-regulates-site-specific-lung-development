...
upstream.dir = "/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/upstream_res"
...
for (sample in samples.rna) {
  ...
  mat = ReadMtx(mtx = paste(upstream.dir, sample, "filtered_matrix", "matrix.mtx.gz", sep = "/"), 
                cells = paste(upstream.dir, sample, "filtered_matrix", "barcodes.tsv.gz", sep = "/"), cell.column = 1, 
                features = paste(upstream.dir, sample, "filtered_matrix", "features.tsv.gz", sep = "/"), feature.column = 2)
...
wrap_plots(plots1, ncol = length(srat.list.rna)/2)
for (sample in names(srat.list.rna)) {
  srat.list.rna[[sample]]$doublet = "singlet"
  # 至少2种算法识别为doublet
  srat.list.rna[[sample]]$doublet[rowSums(srat.list.rna[[sample]]@meta.data[, c("pd_DoubletFinder", "pd_DoubletDetection", "pd_scDblFinder", "pd_Scrublet")] == "doublet") >= 2] = "doublet"
}
...
srat.merge = merge_samples_with_sct(srat.list.rna, assay = "RNA", 
                                    vars.to.regress = c("nFeature_RNA", "pct.mt", "pct.hb", "pct.ribo", "CC.Difference"), 
                                    variable.features.n = 2000, 
                                    add.cell.ids = names(srat.list.rna))
...
noise.spots1 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/mt_high.txt")
noise.spots1$sample = str_split(noise.spots1$V1, "_", simplify = T)[, 1]
noise.spots1 = noise.spots1 %>% dplyr::mutate(cell_id = str_replace(V1, paste0(sample, "_"), ""))
noise.spots2 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/ribo_high.txt")
noise.spots2$sample = str_split(noise.spots2$V1, "_", simplify = T)[, 1]
noise.spots2 = noise.spots2 %>% dplyr::mutate(cell_id = str_replace(V1, paste0(sample, "_"), ""))
noise.spots3 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/low_depth.txt")
noise.spots3$sample = str_split(noise.spots3$V1, "_", simplify = T)[, 1]
noise.spots3 = noise.spots3 %>% dplyr::mutate(cell_id = str_replace(V1, paste0(sample, "_"), ""))
noise.spots4 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/intestine.txt")
noise.spots4$sample = str_split(noise.spots4$V1, "_", simplify = T)[, 1]
noise.spots4 = noise.spots4 %>% dplyr::mutate(cell_id = str_replace(V1, paste0(sample, "_"), ""))
noise.spots5 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/neuron.txt")
noise.spots5$sample = str_split(noise.spots5$V1, "_", simplify = T)[, 1]
noise.spots5 = noise.spots5 %>% dplyr::mutate(cell_id = str_replace(V1, paste0(sample, "_"), ""))
noise.spots6 = read.table("/media/biogenger/D/Projects/LZY/Analysis/alveolar_old_1/alveolar/2merge_annotation/undefined.txt")
noise.spots6$sample = str_split(noise.spots6$V1, "_", simplify = T)[, 1]
...
srat.merge = merge_samples_with_sct(srat.list.rna, assay = "RNA", 
                                    ...
                                    PrepSCT = F)
# cellcycle
cc.list = calc_cellcycle(srat.merge, sample.col = "orig.ident", assay = "SCT", species = "human", nbin = 15)
for (sample in names(cc.list)) {
  srat.list.rna[[sample]]@meta.data = cbind(srat.list.rna[[sample]]@meta.data, cc.list[[sample]])
  srat.list.rna[[sample]]$CC.Difference = srat.list.rna[[sample]]$G2M.Score - srat.list.rna[[sample]]$S.Score
}
...
for (gene in c("SFTPC", "SFTPA1", "SFTPA2", "SFTPB", "AGER", "SCGB3A2", "KRT5", "ACE2", "CTSB", "CTSL", "TMPRSS2")) {
  plots = list()
...
# add info
stress_metadata = qread(paste(tmp.dir1, "stress_metadata.qs", sep = "/"))
srat.merge$orig.ident = factor(srat.merge$orig.ident, levels = samples.rna)
