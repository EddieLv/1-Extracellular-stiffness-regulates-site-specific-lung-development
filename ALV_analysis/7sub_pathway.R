...
df = t(as.matrix(srat.sub[["pathwaysmlm"]]$data)) %>%
  as.data.frame() %>%
  mutate(cluster = Idents(srat.sub)) %>%
...
pheatmap::pheatmap(top_acts_mat, border_color = NA, color = my_color, breaks = my_breaks, cellwidth = 25, cellheight = 20, main = "decoupleR")
CellDimPlot(srat.sub, group.by = "celltype2_final", reduction = "pca_umap", label = T, label_insitu = T)
FeatureDimPlot(srat.sub, assay = "pathwaysmlm", slot = "scale.data", features = c("MAPK", "WNT", "Hypoxia", "TGFb"), reduction = "pca_umap", ncol = 2)
dev.off()
